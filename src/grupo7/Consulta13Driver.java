...
/**
 * Consulta 13 (Grupo 7 - Regresion Lineal con Descenso de Gradiente)
 * Mismo patron iterativo del Grupo 6 (cada epoca = 1 job MapReduce), pero
 * sin sigmoide y con MSE como funcion de perdida.
 *
 * args[0] = dataset original en HDFS
 * args[1] = directorio BASE donde se guarda la salida de cada epoca
 */
public class Consulta13Driver {
	...
	public static void main(String[] args) {
		...
		try {
			for (int epoca = 0; epoca < NUM_EPOCAS; epoca++) {
				...
				JobConf job_conf = new JobConf(Consulta13Driver.class);
				job_conf.setJobName("Consulta13_RegLineal_Epoca" + epoca);
				job_conf.set("pesos", pesosToString(pesos));

				job_conf.setOutputKeyClass(Text.class);
				job_conf.setOutputValueClass(DoubleWritable.class);

				job_conf.setMapperClass(grupo7.Consulta13Mapper.class);
