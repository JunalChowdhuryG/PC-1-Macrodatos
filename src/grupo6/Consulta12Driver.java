...
/**
 * Consulta 12 (Grupo 6 - clasificacion con Regresion Logistica)
 * Mismo patron que Consulta11Driver, con 3 pesos en vez de 4.
 *
 * args[0] = dataset original en HDFS
 * args[1] = directorio BASE donde se guarda la salida de cada epoca
 */
public class Consulta12Driver {

	private static final int NUM_EPOCAS = 30;
	private static final double TASA_APRENDIZAJE = 5.0;
	private static final int NUM_PESOS = 3;
	...
	public static void main(String[] args) {
		...
		try {
			for (int epoca = 0; epoca < NUM_EPOCAS; epoca++) {
				...
				JobConf job_conf = new JobConf(Consulta12Driver.class);
				job_conf.setJobName("Consulta12_LogReg_Epoca" + epoca);
				job_conf.set("pesos", pesosToString(pesos));

				job_conf.setOutputKeyClass(Text.class);
				job_conf.setOutputValueClass(DoubleWritable.class);

				job_conf.setMapperClass(grupo6.Consulta12Mapper.class);
